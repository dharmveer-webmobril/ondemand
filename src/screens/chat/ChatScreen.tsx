...
export default function ChatScreen() {
    ...
    const userDetails = authState.userDetails as { name?: string; profileImage?: string } | null;
    ...
    const { data: chatBookingDetails } = useGetBookingChatDetails(bookingId ?? '');
    useEffect(() => {
        if (chatBookingDetails?.ResponseData?.conversation) {
            if (chatBookingDetails?.ResponseData?.conversation?.participantTwoType === 'serviceProvider') {
                setHeaderData(chatBookingDetails?.ResponseData?.conversation?.participantTwo);
            } else {
                setHeaderData(chatBookingDetails?.ResponseData?.conversation?.participantOne);
            }
        }
    }, [chatBookingDetails]);
    ...
    const recieverId = useMemo(
        () => {
            if (chatBookingDetails?.ResponseData?.conversation?.participantTwoType === 'serviceProvider') {
                return chatBookingDetails?.ResponseData?.conversation?.participantTwo?._id;
            } else {
                return chatBookingDetails?.ResponseData?.conversation?.participantOne?._id;
            }
        },
        [chatBookingDetails]
    );
    ...
    const apiToGifted = useCallback(
        (raw: any[]): IMessage[] => {
            if (!raw || !Array.isArray(raw)) return [];

            return [...raw]
                .reverse()
                .map((msg: any): IMessage => ({
                    _id: msg._id,
                    text: msg.message || '',
    ...
    const onSend = useCallback((newMessages: IMessage[] = []) => {
        ...
        if (!receiverId) {
            console.warn('[Chat] receiverId not loaded yet, message may not be delivered');
        }
                ...
                const formatted = {
                    ...
                    user: {
                        ...
                        avatar:
                            ...
                            data.senderDetails?.profileImage,
                    },
                };
            ...
            >
                <View style={[styles.contentContainer,]}>
                    <ChatHeader
                        ...
                        lastSeen=""
                        bookingId={chatBookingDetails?.ResponseData?.conversation?.bookingId?.bookingId || ''}
                        image={headerData?.profileImage ? { uri: headerData?.profileImage } : imagePaths.no_user_img}
                        onBackPress={() => { }}
                        onCallPress={() => { }}
                        onOptionsPress={() => { }}
                    />
                    <View style={styles.chatContainer}>
                        <GiftedChat
                            ...
                            textInputProps={{ placeholder: 'Type a message...' ,style:{fontSize:16,}}}
