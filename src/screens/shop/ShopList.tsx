...
import { getProvidersByCatAndSubcat, getSpecialOffers } from '../../redux';
    ...
    dispatch(getProvidersByCatAndSubcat({ catId: catId, subcatId: selectedSubCatId === 'all' ? null : selectedSubCatId }))
      .then((result: any) => {
        ...
        if (res && res?.data?.length >= 0) {
          ...
          setIsshopError(t('shop.messages.error'));
          setShopList([]);
        }
      })
      .catch((err: any) => {
    ...
    dispatch(getSpecialOffers())
      .then((result: any) => {
        ...
        if (res && res?.data?.length >= 0) {
          ...
          setISpecialOfferError(t('shop.messages.error'));
          setSpecialOfferList([]);
        }
      })
      .catch((err: any) => {
      ...
      <KeyboardAwareScrollView
        ...
        refreshControl={
          <RefreshControl
            ...
            title={t('shop.shopList.refreshing')}
          />
        }
      >
        <Spacing />
        <View style={styles.paddHori}>
          {subCats && (
            <SubCatList
              ...
              data={[{ _id: 'all', name: t('shop.shopList.allSubCategory') }, ...subCats]} // Localize "All"
            />
          )}
        </View>

        <>
          <AppText style={[styles.specialOffersText, styles.marHori]}>{t('shop.shopList.specialOffers')}</AppText>
