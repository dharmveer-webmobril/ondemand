    ...
    >
      <View style={styles.modalView}>
        <View style={styles.mainView}>
          <Pressable onPress={() => closeModal()} style={styles.crossIcon}>
            <VectoreIcons
              icon="Ionicons"
              name="close"
          ...
          <View style={styles.headingContainer}>
            ...
            />
            <AppText style={styles.heading}>{t('paymentScreen.modal.bookingSuccessful')}</AppText>
          </View>
          <Spacing space={SH(20)} />
          <Pressable style={styles.serviceContainer}>
            <View style={styles.header}>
              <View style={[styles.imageWrapper, boxShadow]}>
                <ImageLoader
                  source={
                    service?.servicesImage && service?.servicesImage?.length > 0
                      ? { uri: service?.servicesImage[0] }
                      : imagePaths.user
                  }
                  resizeMode="cover"
              ...
              <View style={styles.infoContainer}>
                ...
                <AppText style={styles.price}>${service?.price || '0'}</AppText>
                <AppText style={styles.dateTime}>
                  {(slots?.start && slots?.end && `${slots.start}-${slots.end}`) || ''}
                </AppText>
                <AppText style={styles.withText}>
                  {t('paymentScreen.modal.withTeamMember', { fullName: selectedTeamMember?.fullName || '' })}
                </AppText>
              </View>
            </View>
          </Pressable>
          ...
          <Buttons
            ...
            title={t('paymentScreen.modal.submitButton')}
          />
          <Spacing space={SH(8)} />
        </View>
      </View>
